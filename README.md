...
- **`ServiceConf::from_yaml`** — [`moonyaml`](https://github.com/moonbitstack/moonyaml) reads the `etc/*.yaml` into the same `Json` tree the JSON loader produces, so both formats agree field-for-field.
